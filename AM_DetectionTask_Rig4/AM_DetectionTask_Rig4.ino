...
//int outputLevels[2] = {0, 255};
//int outputWeights[2] = {1,4};
...
//Init Exp Defaults
int isiMin = 5000;//as in petersen paper
...
int stimVals[1050];
int stopAfter_n_rewards = 400;  //dont water that mouse too much!
...
if (isRunning == true) {
  ...
  if (trialRunning == false) {
        ...
        if (millis()>=nextTrialStart) {
         ...
         // analogWrite(A11,stimVals[thisTrialNumber]);  //SENT VOLTAGE TO DAQ
 ...
 if (millis()>= rewardPeriodStart && trialRunning == true && (millis()<rewardPeriodEnd))  {
        ...
        if (autoReward == false && lickOccured == true && trialRewarded == false) {  //give water at same time as magnet
          digitalWrite(waterPin, HIGH); //open the water
          valveCloseTime = millis() + valveOpenTime; //mark time to close valve
          trialRewarded = true;
          cumRewards++;
...
if ((millis()>=rewardPeriodEnd) && isResponseWindow == true && trialRunning == true) {
     // Serial.println("fuckity");
      isResponseWindow = false;
   //  resetTrialTime = millis() + 100; 
      resetTrial = true;
//     j if (stimVals[thisTrialNumber] > 0 && trialRewarded == true) {
//      analogWrite(A11,255);  // max volatge equals a hit
//
//      }
//  
//      if (stimVals[thisTrialNumber] == 0 && trialRewarded == true) {
//      analogWrite(A11,0); // voltage = 0 indicated FAILED catch trial
//
//      }
//
//
//      if (stimVals[thisTrialNumber] == 0 && trialRewarded == false) {
//      analogWrite(A11,191); //duty cycle of 75% indicates successful catch trial
//
//      }
//
//
//
//      if (stimVals[thisTrialNumber] > 0 && trialRewarded == false) {
//      analogWrite(A11,64); // duty cycle of 25% indicates MISS
...
if ((millis()>=resetTrialTime) && resetTrial == true && trialRunning == true) {
      ...
      analogWrite(A11,0);  //end transmission
      resetTrial = false;
      trialRunning = false;
      trialRewarded = false;
